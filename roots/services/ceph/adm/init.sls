...
ceph-adm-identity:
  file.managed:
    ...
    - group: {{ user_name }}
    - mode: 600
    - source: salt://bucket/ceph/base/ssh/{{ grains.id }}

ceph-adm-pubkey:
  file.managed:
    - name: /home/{{ pillar.ceph.base.user.name }}/.ssh/id_rsa.pub
    - user: {{ user_name }}
    - group: {{ user_name }}
    - mode: 600
    - source: salt://bucket/ceph/base/ssh/{{ grains.id }}.pub

{% for node in pillar.ceph.cluster.nodes.initial %}
...
create-ceph-cluster:
  cmd.run:
    - name: ceph-deploy new {% for node in pillar.ceph.cluster.nodes.initial %} {{
      node }}{% endfor %}
    - user: {{ user_name }}
    - cwd: {{ ceph_dir }}
...
set-cluster-size:
  file.append:
    ...
    - text:
      - "osd pool default size = {{ pillar.ceph.cluster.size }}"
...
ceph-create-admin-node:
  cmd.wait:
    - name: ceph-deploy admin {{ grains.localhost }}{%
      for node in pillar.ceph.cluster.nodes.initial %} {{ node }}{% endfor %}
    - user: {{ user_name }}
    - cwd: {{ ceph_dir }}
